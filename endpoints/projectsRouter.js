...
router.post("/", (req, res) => {
  ...
  Projects.insert(project)
    ...
    .catch(err => {
      ...
      res
        .status(500)
        .json({ error: "There was a problem adding the new project." });
    });
});
    ...
    if (post) {
      Projects.update(id, { name, description })
        ...
        .catch(err => {
          ...
          res
            .status(500)
            .json({ error: "There was a problem editing project" });
        });
    } else {
  ...
  Projects.get(id)
    .then(project => {
      if (project) {
        ...
        res
          .status(500)
          .json({ error: "There was a problem deleting requested project." });
      }
    })
    .catch(err => {
    ...
    if (project) {
      Projects.getProjectActions(project.id)
        ...
        .catch(err => {
          ...
          res
            .status(500)
            .json({ error: "There was a problem fetching requested actions" });
        });
    } else {
