...
router.get("/", (req, res) => {
  Actions.get()
    ...
    .catch(err => {
      ...
      res
        .status(500)
        .json({ error: "THere was an error fetching requested action." });
    });
});
    ...
    if (action) {
      Actions.remove(id)
        ...
        .catch(err => {
          ...
          res
            .status(500)
            .json({ error: "There was an error removing action." });
        });
    } else {
      res
        .status(404)
        .json({ error: "No action with this ID exists in database." });
    }
  });
});
